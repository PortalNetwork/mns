...
contract MNSResolver {
    ...
    modifier only_owner(bytes32 node) {
        require(mns.owner(node) == msg.sender);
        _;
    }

    /**
     * Constructor.
     * @param mnsAddr The MNS registrar contract.
     */
    function MNSResolver(MNS mnsAddr) public {
        mns = mnsAddr;
    }
    ...
    /**
     * Sets the content hash associated with an MNS node.
     * May only be called by the owner of that node in the MNS registry.
    ...
    /**
     * Returns the content hash associated with an MNS node.
     * Note that this resource type is not standardized, and will likely change
     * in future to a resource type based on multihash.
     * @param node The MNS node to query.
